...
namespace WindowsMotors
{
	public class MSPClient
	{
		...
		{
            //byte[] bytes = { 0x32, 0x16, 0xD8, 0x6E, 0x9E, 0x9C, 0x00, 0x00 };34:B7:DA:F1:10:82
            byte[] bytes = { 0x82, 0x10, 0xF1, 0xDA, 0xB7, 0x34, 0x00, 0x00 };
			...
			while (continueThread)
			{
				try
				{
					...
					else
					{
						...
						{
							GattReadResult readResult = await characteristicRead.ReadValueAsync();

							if (readResult.Status == GattCommunicationStatus.Success)
							{
								// Convert the read value to a byte array
								var reader = DataReader.FromBuffer(readResult.Value);
								if (reader.UnconsumedBufferLength != 0)
								{
									byte[] inputData = new byte[reader.UnconsumedBufferLength];
									reader.ReadBytes(inputData);
									if (ContainsValueGreaterThanZero(inputData))
									{
										System.Diagnostics.Debug.WriteLine("Received bytes: " + BitConverter.ToString(inputData));
